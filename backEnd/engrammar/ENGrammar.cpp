...
std::string engrammar::s(const std::string& verb) {
	if (auto exception = third_person_v->find(verb, 1)) { //handles exceptions
		return std::string(*exception); // Convert string_view to string
	}

	std::string lastChar = verb.substr(verb.size() - 2); //store last character to determine bending of verb
...
std::string engrammar::v_past(std::string& verb, const uint8_t &tense) {
	...
	if (auto exception = irregular_verbs->find(verb, tense)) { //handles exceptions
        return std::string(*exception); // Convert string_view to string
	}
...
std::string engrammar::ing(std::string& word) {
	if (auto exception = ing_verbs->find(word, 1)) { //handles exceptions
		return std::string(*exception); // Convert string_view to string
	}

	std::string lastChar = word.substr(word.size() - 2); //store last character to determine bending of verb
...
int engrammar::initialize() {
    if (irregular_verbs->load(":/xml/grammar/irregular_verbs.xml") != 0) { //load irregular verbs for past tense
		std::cerr << "Failed to load ing exceptions" << std::endl;
		return 1;
	}

    if (third_person_v->load(":/xml/grammar/s.xml") != 0) { //load irregular verbs for present tense third person
		std::cerr << "Failed to load ing exceptions" << std::endl;
		return 1;
	}

    if (ing_verbs->load(":/xml/grammar/ing.xml") != 0) { //load irregular verbs for past tense
		std::cerr << "Failed to load ing exceptions" << std::endl;
		return 1;
	}
