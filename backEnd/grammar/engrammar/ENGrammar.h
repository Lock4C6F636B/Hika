...
namespace engrammar {
    ...
    inline std::unique_ptr<Exceptions<2>> ly_adjectives = std::make_unique<Exceptions<2>>();
    ...
    constexpr TransformMap<std::string_view, std::string_view, 9> lyTransformMap = {
    {"y", "ily"},
    {"ful", "fully"},
    {"le", "ly"},
    {"ic", "ically"},
    ...
    //std::string format_last_verb(std::string &verb, const size_t &format, const std::string &subject);
    ...
    inline const std::string format_conjugate(const std::string &word_a, const size_t &specifier_data) noexcept {
        return word_a + " and ";
    }
    ...
    bool exceptions_empty(const unsigned short &specific_exceptions) noexcept;
